...
public class InventoryImportPatchActivity extends PatchedMainActivity {
    ...
    private static final String PATCH =
        ...
        "function makeFile(b64,name,type){var bytes=bytesFromB64(b64),blob=new Blob([bytes],{type:type||'application/octet-stream'});try{return new File([blob],name||'file',{type:type||'application/octet-stream'});}catch(e){blob.name=name||'file';blob.lastModified=Date.now();return blob;}}"+
        "function finishInventory(b64,name){try{var file=makeFile(b64,name||'inventory.csv','text/csv');if(typeof window.handleInventoryImportFile!=='function')throw new Error('מנגנון ייבוא המלאי אינו זמין');Promise.resolve(window.handleInventoryImportFile(file)).catch(function(e){console.error('[MFIX INVENTORY]',e);toast('ייבוא המלאי נכשל: '+(e&&e.message?e.message:'קובץ לא תקין'),'err');});}catch(e){console.error('[MFIX INVENTORY]',e);toast('ייבוא המלאי נכשל: '+(e&&e.message?e.message:'קובץ לא תקין'),'err');}}"+
        "function finishBackup(b64,name){try{var file=makeFile(b64,name||'mfix-pos-backup.json','application/json');if(typeof window.importBackup!=='function')throw new Error('מנגנון שחזור הגיבוי אינו זמין');window.importBackup({target:{files:[file],value:''}});}catch(e){console.error('[MFIX BACKUP RESTORE]',e);toast('השחזור נכשל: '+(e&&e.message?e.message:'קובץ גיבוי לא תקין'),'err');}}"+
    ...
    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){
        ...
        Uri uri=data.getData();try{InputStream in=getContentResolver().openInputStream(uri);if(in==null)throw new Exception("לא ניתן לפתוח את הקובץ");byte[] bytes=readAll(in);in.close();WebView w=web();if(w==null)throw new Exception("חלון האפליקציה אינו זמין");
            if(requestCode==REQUEST_IMPORT_INVENTORY){String name=displayName(uri);if(name==null||name.trim().isEmpty())name="inventory.csv";if(name.toLowerCase(Locale.ROOT).endsWith(".xlsx")){String csv=xlsxToCsv(bytes);bytes=csv.getBytes(StandardCharsets.UTF_8);name=name.substring(0,name.length()-5)+".csv";}String b64=Base64.encodeToString(bytes,Base64.NO_WRAP);String end="window.mfixReceiveNativeInventoryEnd("+org.json.JSONObject.quote(name)+");";sendChunks(w,"mfixReceiveNativeInventoryChunk",b64,end);}else{String name=displayName(uri);if(name==null||name.trim().isEmpty())name="mfix-pos-backup.json";String b64=Base64.encodeToString(bytes,Base64.NO_WRAP);String end="window.mfixReceiveNativeBackupEnd("+org.json.JSONObject.quote(name)+");";sendChunks(w,"mfixReceiveNativeBackupChunk",b64,end);}
