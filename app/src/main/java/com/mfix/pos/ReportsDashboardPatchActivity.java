...
public class ReportsDashboardPatchActivity extends DataIntegrityPatchActivity {
    private static final String PATCH =
        "(function(){if(window.__mfixReportsDashboardPatchV2)return;window.__mfixReportsDashboardPatchV2=true;"+
        ...
        "function methodAmounts(s){var p=s.payment||s.currentPayment||{};if(Array.isArray(p.parts)&&p.parts.length){var valid=p.parts.map(function(x){return {method:String(x.method||'other'),amount:Number(x.amount||x.value||0)};});var sum=valid.reduce(function(t,x){return t+(Number.isFinite(x.amount)&&x.amount>0?x.amount:0);},0);if(sum>0)return valid;return valid.map(function(x){return {method:x.method,amount:amount(s)/valid.length};});}return [{method:String(p.method||s.paymentMethod||'other'),amount:amount(s)}];}"+
