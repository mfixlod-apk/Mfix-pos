...
/** Runtime user/role layer. Keeps the printer, backup, Yesh Invoice and existing safety layers intact. */
public class UsersPermissionsPatchActivity extends YeshInvoicePatchActivity {
    private static final String USERS_PATCH =
        ...
        "window.mfixAddUser=function(){if(!isManager()){toast('אין הרשאה לניהול משתמשים','err');return;}var n=document.getElementById('mfixUserName').value.trim(),r=document.getElementById('mfixUserRole').value,p=document.getElementById('mfixUserPin').value;if(!n||!validPin(p)){toast('יש להזין שם ו-PIN בן 4–8 ספרות','err');return;}var u=users();u.push({id:'u_'+Date.now(),name:n,role:r,pin:p,active:true});save(u);mfixOpenUsers();toast('המשתמש נוסף','ok');};" +
