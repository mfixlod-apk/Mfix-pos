...
public class ProductEditPatchActivity extends ProductSerialManagementPatchActivity {
    private static final String PATCH =
        "(function(){if(window.__mfixProductEditPatchV2)return;window.__mfixProductEditPatchV2=true;"+
        ...
        "'</div><div class=\\\"field\\\" style=\\\"margin-top:4px\\\"><label style=\\\"font-size:12.5px;font-weight:700;color:var(--gray-700);display:flex;align-items:center;gap:8px\\\"><input id=\\\"mfixProdTrackSerial\\\" type=\\\"checkbox\\\" '+(p.trackSerial?'checked':'')+'> מעקב IMEI / מספר סידורי</label><div class=\\\"muted\\\" style=\\\"font-size:11px;margin-top:4px\\\">הפעל רק למוצרים שבהם כל יחידה מזוהה במספר ייחודי.</div></div></div><div class=\\\"modal-foot\\\"><button class=\\\"btn btn-ghost\\\" type=\\\"button\\\" id=\\\"mfixProdCancel\\\">ביטול</button><button class=\\\"btn btn-primary\\\" type=\\\"button\\\" id=\\\"mfixProdSave\\\">שמור שינויים</button></div>';"+
        "if(window.openModal)window.openModal(html,{wide:false});"+
        "setTimeout(function(){var close=function(){if(window.closeModal)window.closeModal();};var x=document.getElementById('mfixProdClose'),c=document.getElementById('mfixProdCancel');if(x)x.onclick=close;if(c)c.onclick=close;var save=document.getElementById('mfixProdSave');if(save)save.onclick=function(){var name=document.getElementById('mfixProdName').value.trim(),sku=document.getElementById('mfixProdSku').value.trim(),barcode=document.getElementById('mfixProdBarcode').value.trim(),category=document.getElementById('mfixProdCategory').value.trim(),price=Math.max(0,Number(document.getElementById('mfixProdPrice').value)||0),cost=Math.max(0,Number(document.getElementById('mfixProdCost').value)||0),minStock=Math.max(0,Math.floor(Number(document.getElementById('mfixProdMin').value)||0)),trackSerial=!!document.getElementById('mfixProdTrackSerial').checked;if(!name){if(window.toast)window.toast('יש להזין שם מוצר','err');return;}var duplicateSku=sku&&STATE.products.some(function(x){return x!==p&&String(x.sku||'').trim().toLowerCase()===sku.toLowerCase();});var duplicateBarcode=barcode&&STATE.products.some(function(x){return x!==p&&String(x.barcode||x.code||'').trim()===barcode;});if(duplicateSku){if(window.toast)window.toast('המק״ט כבר קיים במוצר אחר','err');return;}if(duplicateBarcode){if(window.toast)window.toast('הברקוד כבר קיים במוצר אחר','err');return;}p.name=name;p.sku=sku;p.barcode=barcode;p.category=category;p.price=Math.round(price*100)/100;p.cost=Math.round(cost*100)/100;p.minStock=minStock;p.trackSerial=trackSerial;p.updatedAt=new Date().toISOString();persist();close();refresh();if(window.toast)window.toast('המוצר עודכן בהצלחה','ok');};},0);"+
        "}"+
        "function draw(){var v=document.getElementById('view-inventory');if(!v)return false;var old=document.getElementById('mfixProductEditTools');if(old)old.remove();if(window.STATE&&window.STATE.currentRole&&window.STATE.currentRole!=='manager')return true;var box=document.createElement('div');box.id='mfixProductEditTools';box.className='card';box.style.marginBottom='12px';var ps=Array.isArray(STATE.products)?STATE.products:[];box.innerHTML='<div class=\\\"section-title\\\">✏️ עריכת מוצרים</div><div class=\\\"muted\\\" style=\\\"font-size:12px;margin-bottom:10px\\\">עריכת פרטי מוצר בלי לשנות את הכמות במלאי.</div><div style=\\\"display:flex;gap:8px;flex-wrap:wrap;align-items:center\\\"><select id=\\\"mfixProductEditSelect\\\" class=\\\"input\\\" style=\\\"max-width:420px\\\"><option value=\\\"\\\">בחר מוצר לעריכה</option>'+ps.map(function(p){return '<option value=\\\"'+esc(p.id)+'\\\">'+esc((p.name||p.sku||p.id)+' · '+(p.sku||'ללא מק״ט'))+'</option>';}).join('')+'</select><button id=\\\"mfixProductEditBtn\\\" class=\\\"btn btn-primary\\\" type=\\\"button\\\">✏️ ערוך</button></div>';v.insertBefore(box,v.firstChild);var sel=document.getElementById('mfixProductEditSelect'),btn=document.getElementById('mfixProductEditBtn');btn.onclick=function(){var p=ps.find(function(x){return String(x.id)===String(sel.value);});if(!p){if(window.toast)window.toast('בחר מוצר לעריכה','err');return;}openEditor(p);};return true;}var tries=0,t=setInterval(function(){tries++;if(draw()||tries>=30)clearInterval(t);},500);console.log('[MFIX] product edit patch V2 active');})();";
