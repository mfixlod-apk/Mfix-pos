...
public class PaymentManagementPatchActivity extends UsersPermissionsPatchActivity {
    private static final String PATCH =
        "(function(){if(window.__mfixPaymentManagementPatchV2)return;window.__mfixPaymentManagementPatchV2=true;"+
        ...
        "function defaultMethod(){try{var s=JSON.parse(localStorage.getItem('mfix_operational_settings_v1')||'{}'),m=String(s.defaultPaymentMethod||'cash');return ['cash','card','transfer','check','other'].indexOf(m)>=0?m:'cash';}catch(e){return 'cash';}}" +
        ...
        "var existing=(window.STATE&&window.STATE.currentPayment)||null;var parts=existing&&Array.isArray(existing.parts)&&existing.parts.length?existing.parts:[{method:existing&&existing.method||defaultMethod(),amount:existing?Number(existing.paid||0):total}];" +
        ...
        "'<div style=\"font-weight:800\">אמצעי תשלום</div><div id=\"mfixPayRows\">'+parts.map(function(p){return row(String(p.method||defaultMethod()),Number(p.amount||0));}).join('')+'</div>'+" +
        ...
        "'<div style=\"margin-top:10px\" class=\"muted\">אפשר לפצל את התשלום בין כמה אמצעים. עודף מותר רק כאשר אמצעי התשלום שמייצר את העודף הוא מזומן.</div></div>'+" +
        ...
        "bind();refresh();" +
        "var add=document.getElementById('mfixPayAdd');if(add)add.onclick=function(){rows.insertAdjacentHTML('beforeend',row(defaultMethod(),0));bind();refresh();};" +
