...
public class GranularPermissionsActivePatchActivity extends ProductEditPatchActivity {
    private static final String PATCH =
        ...
        "function enforce(){if(typeof window.tabAllowed!=='function'||window.__mfixGranularWrapped)return;var old=window.tabAllowed;window.tabAllowed=function(tab){if(STATE.currentRole==='manager')return true;var id=STATE.currentUserId||'',u=read().find(function(x){return String(x.id)===String(id);});if(u&&u.active===false)return tab==='pos'&&false;if(u&&u.permissions&&u.permissions[tab]===false)return false;return old(tab);};window.__mfixGranularWrapped=true;if(typeof forceShellRebuild==='function'&&(!tabAllowed(STATE.activeTab))){STATE.activeTab='pos';forceShellRebuild();}}"+
        ...
        "var n=0;function loop(){n++;card();enforce();if(n<100)setTimeout(loop,800);}loop();console.log('[MFIX] granular permissions + user activation + inventory + checkout + payments + reports chain active');})();";
