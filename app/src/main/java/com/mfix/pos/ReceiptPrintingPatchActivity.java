...
public class ReceiptPrintingPatchActivity extends ProductSerialManagementPatchActivity {
    private static final String PATCH =
        ...
        "function install(){var b=document.querySelector('.mfix-finish-sale');if(!b||b.__mfixReceiptHook)return false;var old=b.onclick;if(typeof old!=='function')return false;b.onclick=function(){var s=settings(),enabled=s.autoPrint!==false;s.autoPrint=false;save(s);old.call(this);if(enabled){setTimeout(function(){try{var a=JSON.parse(localStorage.getItem('mfix_completed_sales_v1')||'[]');var sale=Array.isArray(a)&&a.length?a[a.length-1]:null;if(!sale||typeof window.printDoc!=='function')return;if(!canPrint(s)){if(typeof toast==='function')toast('המכירה נשמרה. אין מדפסת USB ברירת מחדל מאושרת להדפסה אוטומטית','err');return;}window.printDoc(sale.id);}catch(e){console.error('[MFIX] supported USB receipt print failed',e);if(typeof toast==='function')toast('המכירה נשמרה, אך ההדפסה נכשלה','err');}finally{var r=settings();r.autoPrint=enabled;save(r);}},350);}else{var r=settings();r.autoPrint=false;save(r);} };b.__mfixReceiptHook=true;return true;}"+
