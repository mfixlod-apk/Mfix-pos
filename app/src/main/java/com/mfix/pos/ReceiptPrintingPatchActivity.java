...
public class ReceiptPrintingPatchActivity extends ProductSerialManagementPatchActivity {
    private static final String PATCH =
        "(function(){if(window.__mfixReceiptPrintingV4)return;window.__mfixReceiptPrintingV4=true;"+
        ...
        "function printLatest(reason){var s=settings();if(s.autoPrint===false||window.__mfixAutoPrintInFlight)return;if(typeof window.printDoc!=='function'){if(typeof toast==='function')toast('המכירה נשמרה, אך מנגנון ההדפסה אינו זמין','err');return;}var sale=lastSale();if(!sale||!sale.id)return;if(wasPrinted(sale.id))return;if(!canPrint(s)){if(typeof toast==='function')toast('המכירה נשמרה. אין מדפסת USB ברירת מחדל מאושרת','err');return;}window.__mfixAutoPrintInFlight=true;var done=function(){window.__mfixAutoPrintInFlight=false;};try{Promise.resolve(window.printDoc(sale.id)).then(function(){markPrinted(sale.id);if(reason&&typeof console!=='undefined')console.log('[MFIX] receipt printed via '+reason,sale.id);done();}).catch(function(e){console.error('[MFIX] supported USB receipt print failed',e);if(typeof toast==='function')toast('המכירה נשמרה, אך ההדפסה נכשלה','err');done();});}catch(e){console.error('[MFIX] supported USB receipt print failed',e);if(typeof toast==='function')toast('המכירה נשמרה, אך ההדפסה נכשלה','err');done();}}"+
        ...
        "var n=0,t=setInterval(function(){n++;if(install()||n>=80)clearInterval(t);},250);console.log('[MFIX] supported USB receipt printing reliability patch V4 active');})();";
