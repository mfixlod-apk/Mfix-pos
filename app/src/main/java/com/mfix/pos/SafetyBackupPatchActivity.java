...
public class SafetyBackupPatchActivity extends YeshInvoicePatchActivity {
    private static final String PATCH =
        "(function(){if(window.__mfixSafetyBackupV3)return;window.__mfixSafetyBackupV3=true;"+
        "var KEY='mfix_pos_safety_backup_v1';"+
        "var SETTINGS_KEYS=['mfix_operational_settings_v1','mfix_printer_settings_v1','mfix_business_settings_v1','mfix_default_printer_v1'];"+
        ...
        "function localSettings(){var o={};SETTINGS_KEYS.forEach(function(k){try{var v=localStorage.getItem(k);if(v!==null)o[k]=v;}catch(e){}});return o;}"+
        "function restoreLocalSettings(o){if(!o||typeof o!=='object')return;SETTINGS_KEYS.forEach(function(k){if(Object.prototype.hasOwnProperty.call(o,k)){try{localStorage.setItem(k,String(o[k]));}catch(e){}}});}"+
        "function makeSnapshot(){try{if(!window.STATE)return null;return {format:'MFIX_POS_SAFETY_BACKUP',version:3,createdAt:new Date().toISOString(),summary:summary(window.STATE),state:JSON.parse(JSON.stringify(window.STATE)),users:localStorage.getItem('mfix_users_v1'),activeUser:localStorage.getItem('mfix_pos_active_user_v1'),yeshInvoice:localStorage.getItem('mfix_yesh_invoice_v1'),yeshInvoiceOutbox:localStorage.getItem('mfix_yesh_invoice_outbox_v1'),localSettings:localSettings()};}catch(e){console.error('[MFIX SAFETY]',e);return null;}}"+
