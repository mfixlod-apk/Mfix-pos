...
public class ReleaseScopePatchActivity extends ProductEditPatchActivity {
    private static final String PATCH =
        ...
        "var legacy=document.getElementById('mfixInventoryTools');if(legacy)legacy.style.display='none';"+
        ...
        "var role=window.STATE&&String(window.STATE.currentRole||'').toLowerCase();if(role&&role!=='manager'){var add=document.getElementById('mfixSerialAdd');if(add)add.disabled=true;document.querySelectorAll('[data-mfix-serial-del]').forEach(function(b){b.disabled=true;});}"+
        "}"+
