...
public class PrinterQueuePatchActivity extends PrinterManagementPatchActivity {
    private static final String PATCH =
        "(function(){if(window.__mfixPrinterQueueV2)return;window.__mfixPrinterQueueV2=true;"+
        "var queue=[],busy=false,lastSaleId='';"+
        ...
        "function refreshControls(){var host=document.getElementById('mfixPrinterManagement');if(!host)return;var b=document.getElementById('mfixPrinterQueueClear');if(!b){b=document.createElement('button');b.id='mfixPrinterQueueClear';b.className='btn btn-ghost';b.textContent='🗑️ נקה תור';var row=host.querySelector('div[style*="display:flex"]');if(row)row.appendChild(b);b.onclick=function(){var dropped=queue.length;queue=[];status(dropped?'🗑️ הוסרו '+dropped+' עבודות ממתינות':'🖨️ אין עבודות ממתינות','ok');};}b.disabled=queue.length===0;}"+
        "function pump(){refreshControls();if(busy||!queue.length){if(!busy&&!queue.length)status('🖨️ תור ההדפסה ריק','ok');return;}busy=true;var job=queue.shift();refreshControls();status('🖨️ שולח להדפסה...'+(queue.length?' נשארו '+queue.length+' בקשות':''));Promise.resolve().then(function(){return job.original(job.saleId);}).then(function(){busy=false;lastSaleId=job.saleId||'';if(queue.length)status('🖨️ הבקשה נשלחה — עוברים לבאה','ok');else status('🖨️ הבקשה נשלחה; התור ריק','ok');refreshControls();setTimeout(pump,120);}).catch(function(e){busy=false;console.error('[MFIX PRINT QUEUE]',e);status('⚠️ בקשת הדפסה נכשלה — '+String(e&&e.message||e),'err');refreshControls();setTimeout(pump,120);});}"+
        "function install(){if(typeof window.printDoc!=='function'){setTimeout(install,300);return;}if(window.printDoc.__mfixQueued)return;var original=window.printDoc;function queuedPrintDoc(saleId){if(!saleId)return original.apply(this,arguments);if(busy&&saleId===lastSaleId)return;for(var i=0;i<queue.length;i++)if(queue[i].saleId===saleId)return;queue.push({saleId:saleId,original:original});status('🖨️ בקשת הדפסה נוספה לתור ('+queue.length+')');refreshControls();pump();}queuedPrintDoc.__mfixQueued=true;window.printDoc=queuedPrintDoc;refreshControls();console.log('[MFIX] printer request queue V2 active');}"+
        "install();})();";
