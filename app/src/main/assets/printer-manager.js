(function(){
  ...
  function testData(){ return b64([0x1b,0x40,0x1b,0x61,0x01].concat(Array.from(new TextEncoder().encode('MFIX PRINTER TEST\n----------------\nUSB ESC/POS OK\n\n')), [0x1b,0x61,0x00,0x0a,0x1d,0x56,0x00])); }
  function api(){ return window.AndroidPrinter || null; }
  function list(){ try { var a=JSON.parse(api().listUsbPrinters()||'[]'); return Array.isArray(a)?a:[]; } catch(e){ return []; } }
  ...
  function setDefaultPrinter(p){
    ...
    var s=printerSettings(); s.device=id; s.enabled=true; if(!s.paperMode) s.paperMode='80MM'; localStorage.setItem('mfix_printer_settings_v1', JSON.stringify(s));
    try { if(window.STATE && window.STATE.settings){ window.STATE.settings.defaultPrinterId=id; var printers=Array.isArray(window.STATE.settings.printers)?window.STATE.settings.printers:[]; var exists=printers.some(function(x){return String(x.id)===id;}); if(!exists) printers.push({id:id,name:String(p.name||id),type:'USB',address:id}); window.STATE.settings.printers=printers; if(typeof window.persist==='function') window.persist('settings'); } } catch(e){ console.error('[MFIX PRINTER DEFAULT SYNC]',e); }
    return true;
  }
  function savePaperMode(mode){ mode=String(mode||'80MM').toUpperCase()==='58MM'?'58MM':'80MM'; var s=printerSettings(); s.paperMode=mode; localStorage.setItem('mfix_printer_settings_v1',JSON.stringify(s)); try{ if(window.STATE&&window.STATE.settings){window.STATE.settings.printerPaperMode=mode;if(typeof window.persist==='function')window.persist('settings');} }catch(e){} }
  function selectedPrinter(ps){ var id=defaultPrinterId(); if(!id) return null; return ps.find(function(x){return String(x.id||'')===id || String(x.name||'')===id;}) || null; }
  function open(){
    var a=api(); if(!a){ alert('הדפסת USB אינה זמינה בגרסת המכשיר הזו'); return; }
    var printers=list(), currentId=defaultPrinterId();
    ...
    document.querySelectorAll('[data-mfix-action]').forEach(function(btn){btn.onclick=function(){
      ...
      try{
        var p=list().find(function(x){return String(x.id)===String(id);}); if(!p){ alert('המדפסת כבר אינה מחוברת'); return; }
        if(action==='select'){ setDefaultPrinter(p); alert('המדפסת נבחרה כברירת מחדל: '+(p.name||p.id)); document.getElementById('mfixPrinterOverlay').remove(); open(); }
        else if(action==='test'){ if(!p.authorized){ alert('למדפסת זו אין כרגע הרשאת USB. אשר את הרשאת ה-USB במכשיר ונסה שוב.'); return; } if(defaultPrinterId()!==String(p.id) && defaultPrinterId()!==String(p.name||'')){ alert('בחר תחילה את המדפסת כברירת מחדל.'); return; } a.printEscPosToDevice(id,testData()); }
        else if(action==='drawer'){ if(!p.authorized){ alert('למדפסת זו אין כרגע הרשאת USB.'); return; } if(defaultPrinterId()!==String(p.id) && defaultPrinterId()!==String(p.name||'')){ alert('בחר תחילה את המדפסת כברירת מחדל.'); return; } a.openCashDrawer(id); }
  ...
  function installCheckoutDrawerHook(){
    ...
    window.finalizeSale=async function(){
      ...
      try{
        var s=printerSettings(); if(s.drawer!==true) return result;
        var st=window.STATE; if(!st || !Array.isArray(st.sales) || st.sales.length<=beforeCount) return result;
        ...
        var bridge=api(); if(!bridge || typeof bridge.openCashDrawer!=='function') return result;
        var printers=list(), p=selectedPrinter(printers); if(p && p.authorized && p.id) bridge.openCashDrawer(p.id);
      }catch(e){ console.error('[MFIX CASH DRAWER]',e); }
      return result;
    };
  }
  function install(){
    if(!document.body) return; var old=document.getElementById('mfixPrinterManagerButton'); if(old) old.remove();
    var b=document.createElement('button'); b.id='mfixPrinterManagerButton'; b.textContent='🖨️'; b.title='ניהול מדפסות'; b.style.cssText='position:fixed;left:16px;bottom:16px;z-index:9000;width:48px;height:48px;border:0;border-radius:50%;background:#2158cf;color:#fff;font-size:22px;box-shadow:0 6px 18px rgba(0,0,0,.2);cursor:pointer'; b.onclick=open; document.body.appendChild(b); installCheckoutDrawerHook();
  }
